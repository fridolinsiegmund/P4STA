sudo apt update && sudo apt install xz-utils docker.io
...
# SDE_FILE=bf_sde_docker/bf-sde-9.13.4.tar.xz
# if test -f "$SDE_FILE"; then
#     echo "$SDE_FILE exists."
#     sudo docker build --no-cache --pull --progress=plain -t bf_sde:9.13.4 bf_sde_docker
...
sudo docker build --no-cache --pull --progress=plain -t management core_docker
...
sudo docker build --no-cache --pull --progress=plain -t mininet_bmv2 bmv2_mn_docker
if [ $? -ne 0 ]; then
