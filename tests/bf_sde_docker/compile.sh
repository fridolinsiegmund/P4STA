#!/bin/bash
#
# use : ./custom.sh   --out-dir $PWD/compile/   --with-tofino   tofino_stamper_v1_4_0.p4   P4_VERSION=p4_16   P4_ARCHITECTURE=tna
#
# modified version of https://github.com/P4ica/tools/blob/main/p4_build.sh
#
# p4_build.sh
#
...
# Optional custom output directory. If set with --out-dir, build/log/install
# artifacts are kept under this folder and <P4_NAME>.conf is copied to the
# top level of that folder. Example: --out-dir /compile
P4_OUT_DIR=
...
Supported options:
  ...
  ========
    ...
    --out-dir=dir
                     Install the compiled P4 artifacts into a custom directory,
                     e.g. --out-dir /compile. The generated <P4_NAME>.conf is
                     additionally copied directly into the top level of dir.
...
#            tna.p4                       -- p4_16/tna
#            tofino1*.p4                  -- p4_16/tna
#            t2na.p4                      -- p4_16/t2na
#            tofino2*.p4                  -- p4_16/t2na
...
check_program() {
    if p4_includes $P4_REALPATH tofino/intrinsic_metadata.p4 inc_tofino; then
        ...
        p4_includes $P4_REALPATH 'tofino1.*\.p4'       inc_tofino1arch
        p4_includes $P4_REALPATH 't2na\.p4'            inc_t2na
        p4_includes $P4_REALPATH 'tofino2.*\.p4'       inc_tofino2arch
...
prepare_custom_output_dir() {
    if [ -z "$P4_OUT_DIR" ]; then
        return 0
    fi

    mkdir -p "$P4_OUT_DIR"
    P4_OUT_DIR=$(realpath "$P4_OUT_DIR")
    echo "Using custom output directory ${P4_OUT_DIR}"
}

set_p4_output_paths() {
    # Must be called after P4_NAME and P4_TARGET are known.
    if [ -n "$P4_OUT_DIR" ]; then
        # Keep temporary build/logs below the custom output directory, but not
        # directly in its top level. The install prefix is the custom directory.
        P4_BUILD="$P4_OUT_DIR/.build/p4-build/${P4_TARGET}/${P4_NAME}${P4_SUFFIX}"
        P4_LOGS="$P4_OUT_DIR/.logs/p4-build/${P4_TARGET}/${P4_NAME}${P4_SUFFIX}"
        P4_INSTALL="$P4_OUT_DIR"
    else
...
escape_sed_replacement() {
    # Escape a string so it can be safely used as the replacement side of sed s#old#new#g.
    printf '%s' "$1" | sed 's#[\&]#\\&#g'
}

escape_sed_pattern() {
    # Escape a string so it can be safely used as the pattern side of sed s#old#new#g.
    printf '%s' "$1" | sed 's#[\.^$*+?()[{|]#\\&#g'
}

rewrite_conf_paths_to_output_dir() {
    local conf_file="$1"

    # Some p4-build/CMake generated configs still contain absolute paths to the
    # active SDE install tree even when CMAKE_INSTALL_PREFIX/--prefix is changed.
    # For a custom output build, the P4 artifacts should be loaded from P4_INSTALL.
    if [ -n "$SDE_INSTALL" ]; then
        sed -i "s#$(escape_sed_pattern "$SDE_INSTALL")#$(escape_sed_replacement "$P4_INSTALL")#g" "$conf_file"
    fi

    if [ -n "$SDE" ]; then
        sed -i "s#$(escape_sed_pattern "$SDE/install")#$(escape_sed_replacement "$P4_INSTALL")#g" "$conf_file"
    fi

    # CMake/p4-build may generate valid-looking but ambiguous relative artifact
    # paths, e.g. "share/tofinopd/<program>/pipe/context.json".  bf_switchd can
    # resolve those relative to its runtime/install directory, not necessarily
    # relative to this top-level conf file.  Make the P4 artifact paths absolute
    # so the top-level conf is self-contained.
    if [ -n "$P4_OUT_DIR" ]; then
        P4_INSTALL="$P4_INSTALL" perl -0pi -e '
            my $prefix = $ENV{"P4_INSTALL"};
            $prefix =~ s{/+$}{};
            s{("(?:bfrt-config|context|config|path)"\s*:\s*")(?!(?:/|[A-Za-z][A-Za-z0-9+.-]*:))}{$1$prefix/}g;
        ' "$conf_file"
    fi
}

verify_conf_uses_output_dir_for_p4_artifacts() {
    local conf_file="$1"
    local bad=0

    # The switchd config may still contain SDE paths for runtime libraries, but
    # these P4 artifact fields must not point back into SDE_INSTALL.
    if grep -nE '"(context|config|bfrt-config)"' "$conf_file" | grep -E "${SDE_INSTALL:-__NO_SDE_INSTALL__}|${SDE:-__NO_SDE__}/install"; then
        echo "ERROR: $conf_file still points P4 artifacts to SDE/SDE_INSTALL instead of $P4_INSTALL"
        bad=1
    fi

    if grep -nE '"(context|config|bfrt-config)"' "$conf_file" | grep -v "$P4_INSTALL"; then
        echo "ERROR: Some P4 artifact paths above do not contain $P4_INSTALL"
        bad=1
    fi

    # Check the most important generated files early so switchd does not fail
    # later with a less obvious path-resolution error.
    local missing=0
    while IFS= read -r artifact_path; do
        if [ -n "$artifact_path" ] && [ ! -f "$artifact_path" ]; then
            echo "ERROR: Referenced artifact does not exist: $artifact_path"
            missing=1
        fi
    done < <(sed -nE 's/.*"(context|config|bfrt-config)"[[:space:]]*:[[:space:]]*"([^"]+)".*/\2/p' "$conf_file")
...
copy_conf_to_output_top_level() {
    if [ -z "$P4_OUT_DIR" ]; then
        return 0
    fi

    local conf_src=""
    local conf_dst="$P4_OUT_DIR/${P4_NAME}.conf"

    # p4-build/CMake usually installs the switchd config here.
    for candidate in \
        "$P4_INSTALL/share/p4/targets/${P4_TARGET}/${P4_NAME}.conf" \
        "$P4_INSTALL/share/p4/targets/tofino/${P4_NAME}.conf" \
        "$P4_INSTALL/share/p4/targets/tofino2/${P4_NAME}.conf" \
        "$P4_INSTALL/share/tofinopd/${P4_NAME}/${P4_NAME}.conf"
    do
        if [ -f "$candidate" ]; then
            conf_src="$candidate"
            break
    ...
    if [ -z "$conf_src" ]; then
        echo "WARNING: Could not find ${P4_NAME}.conf below $P4_INSTALL; nothing copied to $conf_dst"
        return 0
    fi

    cp "$conf_src" "$conf_dst"
    rewrite_conf_paths_to_output_dir "$conf_dst"
    verify_conf_uses_output_dir_for_p4_artifacts "$conf_dst"
    echo "Top-level conf: $conf_dst"
}
build_p4_prog() {
    local cmd
