...
class TargetImpl(AbstractTarget):
    ...
    def get_port_mapping(self, ssh_ip, data_plane_program_name, tofino_grpc_obj):
        if self.port_mapping is None:
            ...
            try:
                if tofino_grpc_obj != None and "p4_connected" in dir(tofino_grpc_obj) and tofino_grpc_obj.p4_connected:
                    self.logger.info("Open P4 connection reused")
                    interface = tofino_grpc_obj
                else:
                    interface, close_interface, error = \
                        grpc_interface.TofinoInterface.get_or_create_bound_instance(
                            ssh_ip, 0, self.logger, data_plane_program_name)
                    if error != "":
                ...
                if interface is not None and close_interface:
                    interface.teardown()
    ...
    def update_portmapping(self, cfg, tofino_grpc_obj=None):
        ...
        if "session_cp_real" in cfg:
            real_port = cfg["session_cp_real"].replace("-", "0")
            if real_port in port_map:
                cfg["session_cp"] = port_map[real_port]

        if "session_cp2_real" in cfg:
            real_port = cfg["session_cp2_real"].replace("-", "0")
            if real_port in port_map:
                cfg["session_cp2"] = port_map[real_port]

        print("Updated portmapping: " + str(port_map))
    ...
    def deploy(self, cfg, tofino_grpc_obj=None):
        ...
        try:
            if tofino_grpc_obj != None and tofino_grpc_obj.connection_established and tofino_grpc_obj.p4_connected:
                ...
                interface, close_interface, total_error = \
                    grpc_interface.TofinoInterface.get_or_create_bound_instance(
                        cfg["stamper_ssh"], 0, self.logger, cfg["program"])
        ...
        if len(total_error) < 2 and interface is not None:
            try:
                ...
                if cfg["session_cp_real"] != "bf_pci0" and cfg["session_cp_real"] != "320":
                    if cfg["session_cp_real"] != "":
                        hosts.append({"p4_port": cfg["session_cp"],
                                    "real_port": cfg["session_cp_real"],
                                    "speed": cfg["session_cp_speed"],
                                    "fec": cfg["session_cp_fec"],
                                    "an": cfg["session_cp_an"]})
                        
                if cfg["session_cp2_real"] != "bf_pci0" and cfg["session_cp2_real"] != "320":
                    if cfg["session_cp2_real"] != "":
                        hosts.append({"p4_port": cfg["session_cp2"],
                                    "real_port": cfg["session_cp2_real"],
                                    "speed": cfg["session_cp2_speed"],
                                    "fec": cfg["session_cp2_fec"],
                                    "an": cfg["session_cp2_an"]})
    ...
    def deploy_tables(self, cfg, interface):
        ...
        try:
            ...
            # t_l1_forwarding #
            ###################
            # Note: all forwarding table entries may be created/deleted here
            # also in load generator modules (e.g. tofino loadgen) and session modules when running tests
            t_l1_forwarding = "pipe.SwitchIngress.t_l1_forwarding"
            if cfg['forwarding_mode'] == "1":
                try:
                    for dut in cfg["dut_ports"]:
                        if dut["use_port"] == "checked":
                            # l1 forwarding tables are required for external loadgens such as e.g. iperf3 loadgens
                            # can NOT be used together with session modules
                    ...
                    if dut["use_port"] == "checked":
                        interface.add_to_table(t_l2_forwarding, [
                            ...
                            ["hdr.ethernet.dstAddr",
                             self.mac_str_to_int("ff:ff:ff:ff:ff:ff")]],
                                               [["group", dut["id"] + 1]],
                                ...
                                interface.add_to_table(
                                    t_l2_forwarding, [
                                        ["ig_intr_md.ingress_port",
                                         int(dut["p4_port"])],
                                        ["hdr.ethernet.dstAddr",
                                         self.mac_str_to_int(host["loadgen_mac"])]
                                    ],
                                    [
                                        ["egress_port",
                                         int(host["p4_port"])]
        ...
        try:
            ...
            try:
                interface.add_to_table(
                    ...
                    [
                        ["dst", self.mac_str_to_int(cfg["ext_host_mac"])],
        ...
        # Disabled for 1.4.0 => tables are added in session module
        # try:
        #     #############################
        #     #    for external host      #
        #     #############################
    ...
    def read_stamperice(self, cfg, tofino_grpc_obj=None):
        ...
        try:
            ...
            if close_interface and interface is not None:
                interface.teardown()

        return cfg

    def stamper_status(self, cfg, tofino_grpc_obj=None):
            ...
            if len(lines) > 0 and lines[0].isdigit() and int(lines[0]) > 0:
                ...
                if tofino_grpc_obj != None and \
                    ...
                    interface = grpc_interface.TofinoInterface.get_bound_instance(
                        cfg["stamper_ssh"], 0, cfg["program"])

                if interface is None:
                    interface, close_interface, error = \
                        grpc_interface.TofinoInterface.get_or_create_bound_instance(
                            cfg["stamper_ssh"], 0, self.logger,
                            cfg["program"])
                    if error != "":
    ...
    def get_server_install_script(self, user_name, ip, target_specific_dict={}):
        ...
        if "sde" in target_specific_dict and len(target_specific_dict["sde"]) > 1:
            ...
            if sde_path == "":
                self.logger.warning("SDE Path not found on Tofino, using /opt/bf-sde-9.13.4")
                sde_path = "/opt/bf-sde-9.13.4"
        ...
        def get_session_module_setup_cfg():
            session_module_name = target_specific_dict.get("session_module")
            if session_module_name is None or session_module_name == "NONE":
                return []

            session_modules_path = os.path.join(project_path, "session_modules")
            for module_dir in os.listdir(session_modules_path):
                cfg_path = os.path.join(
                    session_modules_path, module_dir, "module_cfg.json")
                if not os.path.isfile(cfg_path):
                    continue
                with open(cfg_path, "r") as f:
                    module_cfg = json.load(f)
                if module_cfg.get("name") == session_module_name:
                    return module_cfg.get("config", {}).get(
                        "setup_specific", [])
            self.logger.warning(
                "No module_cfg.json found for session module " +
                str(session_module_name))
            return []

        session_module_setup_cfg = get_session_module_setup_cfg()
        dyn_cfg_options_by_key = {}
        for opt in session_module_setup_cfg:
            if opt.get("type") == "checkbox-group":
                dyn_cfg_options_by_key[opt["target_key"]] = opt

        def get_selected_dyn_cfg():
            opt = dyn_cfg_options_by_key.get("dyn_cfg", {})
            allowed_fields = set(
                tuple(field) for field in opt.get("values_to_select", []))
            selected_values = target_specific_dict.get(
                "dyn_cfg", opt.get("selected_values", []))
            dyn_cfg = []
            for selected_value in selected_values:
                field = tuple(selected_value)
                if len(field) == 3 and all(isinstance(part, str) for part in field) \
                        and field in allowed_fields:
                    dyn_cfg.append(list(field))
                else:
                    self.logger.warning(
                        "Ignoring invalid dynamic session compile field: " +
                        str(selected_value))
            return dyn_cfg

        dyn_cfg = get_selected_dyn_cfg()
        ...
        for c in dyn_cfg:
            ...
            if c[0] != "" and c[1] != "" and c[2] != "":
                p4c_flags = p4c_flags + " -D P4_HEADER" + dx +"=" + c[0] + " -D P4_FIELD" + dx + "=" + c[1] + " -D P4_F_TYPE" + dx + "=" + c[2]


        compile_p4_src = "export SDE_INSTALL=" + sde_path + "/install\n" \
                         "cd /home/" + user_name + "/p4sta/stamper/tofino1/\n" \
                         "mkdir -p compile\n" \
                         "$SDE_INSTALL/bin/bf-p4c -v -g "+ p4c_flags +" -o $PWD/compile/ tofino_stamper_v1_4_0.p4\n"
        ...
        lst.append('  scp ' + dir_path + '/p4_files/v1.4.0/header_tofino_stamper_v1_4_0.p4 ' + user_name + '@' + ip + ':/home/' + user_name + '/p4sta/stamper/tofino1/')
        lst.append('  scp ' + dir_path + '/p4_files/v1.4.0/session_mod_actions.p4 ' + user_name + '@' + ip + ':/home/' + user_name + '/p4sta/stamper/tofino1/')
        lst.append('  scp ' + dir_path + '/p4_files/v1.4.0/tofino_stamper_v1_4_0.p4 ' + user_name + '@' + ip + ':/home/' + user_name + '/p4sta/stamper/tofino1/')
    ...
    def get_live_metrics(self, cfg, tofino_grpc_obj=None):
        ...
        if tofino_grpc_obj == None:
            ...
            if error != "":
                self.logger.warning(error)
            self.logger.debug("Probing gRPC connection to Tofino... Established: " + str(interface.connection_established))
        else:
            interface = tofino_grpc_obj
            if tofino_grpc_obj.p4_program == "":
                interface.bind_p4_name(cfg["program"])
