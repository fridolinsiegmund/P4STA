...
header icmp_t {
	...
	bit<16> identifier;
	bit<16> seqno;
	// tcp_options_128bit_custom_t optional_data;
}
...
header ext_host_stats_t {
	bit<16> paket_len;
	bit<32> sess_identifier;
	//bit<9> ingress_port;
}

struct p4sta_metadata_t {
	bit<16> tcpLen;
	bit<1> updateChecksum;
	bit<1> updateEgressIPChecksum;
	...
	bit<4> new_offset;
	bit<16> ipv4_paketlen_temp;
	bit<16> outer_ipv4_paketlen_temp;
}

// temp fields to enable 32+32 bit registers as 64 bit
struct timestamp_metadata_t {
	bit<32> delta_sum_low;
	bit<32> delta;			// stores calculatet latency for packet
	bit<32> register_pkts_sum_low; // for register count delta packets 
}

struct headers_t {
	pktgen_timer_header_t pkg_gen_timer;
	ptp_mac_hdr_t ptp;
	ethernet_t ethernet;
	vlan_tag_t vlan;
	vlan_qinq_tag_t vlan_qinq;
	pppoe_t pppoe;
	...
	//exthost (customized) headers to capsulate packet beginning from tcp/udp
	udp_t exthost_udp;
	ext_host_stats_t ext_host_stats;
...
struct my_metadata_t {
	p4sta_metadata_t p4sta_metadata;
	timestamp_metadata_t timestamp_metadata;
	bit<8> header_offset; //variable which describes additional offset to minimal packet for PPPoE or GTP-U
	bit<8> l2_header_offset; //variable which describes offset to ethernet header, e.g. in case of VLAN => both offsets are added in control
	bit<32> logical_index;
	#ifdef DO_METER
	bit<8> meter_color;
	#endif
	bit<16> etherType;
	bit<16> session_identifier16; // session_identifier can be either PPPoE session ID ..., is 0 if upstream
	bit<32> session_identifier32; // session_identifier can be either GTP-U TEID (downstream), is 0 if upstream
	bit<32> session_identifier_ip; // only used for upstream traffic where session identifier is source IP address
}
