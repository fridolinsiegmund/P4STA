...
#include <t2na.p4>
#define LOG_INDX_MAX_SIZE 404381 //403456 //0x63000 // 0x4d000 //405504 // tofino 2 (0x63000)
#else
#include <tna.p4>
#define LOG_INDX_MAX_SIZE 135168 // tofino 1 (0x21000) 
#endif

#include "header_tofino_stamper_v1_4_0.p4"

#define MAC_STAMPING
//#define P4STA_DEBUG_COUNTERS
//#define GTP_ENCAP
//#define PPPOE_ENCAP
...
parser SwitchIngressParser(packet_in packet, out headers_t hdr, out my_metadata_t meta, out ingress_intrinsic_metadata_t ig_intr_md) {
	...
	state start {
		...
		meta.p4sta_metadata.updateChecksum = 0;
		meta.p4sta_metadata.do_multicast = 0;
		meta.p4sta_metadata.added_empty_header = 0;
		meta.logical_index = 0;
	...
	state pre_parse_ip_prot {
		transition select(packet.lookahead<ipv4_udp_t>().protocol) {
			8w0x11:pre_parse_gtpu_srcport; // some UPF impl. may use a random source port despite the standard specifying 2152 for src and dst - then skip to gtpu_dstport here
			default: parse_ipv4;
		}
	}
	...
	state parse_qinq_vlan {
		...
		meta.etherType = hdr.vlan_qinq.etherType;
	...
	state parse_icmp {
		...
		meta.p4sta_metadata.checksum_l4_tmp = icmp_checksum.get();
	...
	state parse_tcp {
		...
		meta.p4sta_metadata.checksum_l4_tmp = tcp_checksum.get();
	...
	state parse_udp {
		...
		transition select(hdr.udp.len) {
			// prevent parsing of udp packets smaller than 24 bytes, as we need at least 16 bytes for timestamp header and 8 bytes for udp header
			// otherwise, e.g., Ethernet padding could be parsed here setting tcp_options_128bit_custom wrongly to valid!
			0 .. 23: accept;
			default: parse_udp_p4sta_timestamps;
		}

		//packet.extract(hdr.tcp_options_128bit_custom);
		//udp_checksum.subtract({hdr.tcp_options_128bit_custom});
		//meta.p4sta_metadata.checksum_l4_tmp = udp_checksum.get();
		//transition accept;
	}

	state parse_udp_p4sta_timestamps {
		packet.extract(hdr.tcp_options_128bit_custom);
		udp_checksum.subtract({hdr.tcp_options_128bit_custom});
		meta.p4sta_metadata.checksum_l4_tmp = udp_checksum.get();
		transition accept;
...
control SwitchIngress(
	...
	#ifdef P4STA_DEBUG_COUNTERS
	// Debugging counters
	Counter<bit<32>, PortId_t>(512, CounterType_t.PACKETS_AND_BYTES) dbg_ipv4_valid_counter;
	Counter<bit<32>, PortId_t>(512, CounterType_t.PACKETS_AND_BYTES) dbg_tcp_valid_counter;
	Counter<bit<32>, PortId_t>(512, CounterType_t.PACKETS_AND_BYTES) dbg_tcp_custom_valid_counter;
	Counter<bit<32>, PortId_t>(512, CounterType_t.PACKETS_AND_BYTES) dbg_tcp_custom_0f10_counter;
	Counter<bit<32>, PortId_t>(512, CounterType_t.PACKETS_AND_BYTES) dbg_tcp_add_empty_counter;
	Counter<bit<32>, PortId_t>(512, CounterType_t.PACKETS_AND_BYTES) dbg_tcp_timestamp2_apply_counter;
	Counter<bit<32>, PortId_t>(512, CounterType_t.PACKETS_AND_BYTES) dbg_tcp_timestamp2_action_counter;
	Counter<bit<32>, PortId_t>(512, CounterType_t.PACKETS_AND_BYTES) dbg_timestamp2_mc_counter;
	#endif
	...
	RegisterAction<bit<32>, bit<32>, bit<1> >(multi_counter_register) multi_counter_register_action = {
		...
		void apply(inout bit<32> counter, out bit<1> read_value){
			if(counter >= (bit<32>)meta.p4sta_metadata.threshold_multicast) {
				read_value = 1w0x1;
				counter = 32w0x0;
			} else {
				read_value = 1w0x0;
	...
	action add_timestamp_header_tcp(){
		#ifdef P4STA_DEBUG_COUNTERS
		dbg_tcp_add_empty_counter.count(ig_intr_md.ingress_port);
		#endif
		...
		meta.p4sta_metadata.new_offset = (bit<4>)(hdr.tcp.dataOffset + 4w0x4); //0x8;//new_offset;
		hdr.ipv4.paketlen = hdr.ipv4.paketlen + 16w0x10; // + 16 byte payload (tcp header + payload)

		meta.p4sta_metadata.added_empty_header = 1w0x1;
		meta.p4sta_metadata.updateChecksum = 1w0x1;
	}

	action add_timestamp2_tcp(bit<16> threshold) {
		#ifdef P4STA_DEBUG_COUNTERS
		dbg_tcp_timestamp2_action_counter.count(ig_intr_md.ingress_port);
		#endif
		meta.p4sta_metadata.threshold_multicast = threshold;  // threshold for multicast how many packets get replicated
		...
		meta.p4sta_metadata.updateChecksum = 1w0x1;
		meta.p4sta_metadata.timestamp2 = 1w0x1;
	}
	...
	action add_timestamp2_udp(bit<16> threshold) {
		meta.p4sta_metadata.threshold_multicast = threshold;
	...
	action check_udp_size(){
		meta.p4sta_metadata.udp_length_cutted = (bit<8>)(hdr.udp.len >> 5); // ignore first 5 bits, 6th bit = 32byte size = if rest is bigger than 0 udp packet is bigger than 32 byte
	}
...
///////// tables /////////
	...
	// for forwarding e.g. session control plane traffic to session module, but also arp, icmp, bng .. but not p4sta load flows
	// cannot reuse t_l1_forwarding as we restrict this table in apply block
	table t_l1_forwarding_session_all_except_load {
		key = {
			ig_intr_md.ingress_port : exact;
		}
		actions = {
	...
	apply {
		...
		// only apply forwarding for non-p4sta load packets (indicated by missing tstamp header)
		// if problems occur here: check further for ARP headers etc
		if(!hdr.tcp_options_128bit_custom.isValid()) {
			t_l1_forwarding_session_all_except_load.apply();
		}

		if(hdr.ipv4.isValid()) {
			#ifdef P4STA_DEBUG_COUNTERS
			dbg_ipv4_valid_counter.count(ig_intr_md.ingress_port);
			#endif
			if((bit<32>)hdr.tcp_options_128bit_custom.timestamp2 != 0x1){
			...
			if(hdr.tcp.isValid()) {
				#ifdef P4STA_DEBUG_COUNTERS
				dbg_tcp_valid_counter.count(ig_intr_md.ingress_port);
				if(hdr.tcp_options_128bit_custom.isValid()){
					dbg_tcp_custom_valid_counter.count(ig_intr_md.ingress_port);
				}
				if(hdr.tcp_options_128bit_custom.myType == 16w0x0f10){
					dbg_tcp_custom_0f10_counter.count(ig_intr_md.ingress_port);
				}
				#endif
				...
				if(hdr.tcp.dataOffset <= 0xB){
					...
					if(meta.p4sta_metadata.new_offset > 0){
						// setting the dataOffset field in action t_add_empty_timestamp_tcp is too much
						hdr.tcp.dataOffset = meta.p4sta_metadata.new_offset;
					}
				}
				if(hdr.tcp_options_128bit_custom.myType == 16w0x0f10 && meta.p4sta_metadata.added_empty_header == 0){
					#ifdef P4STA_DEBUG_COUNTERS
					dbg_tcp_timestamp2_apply_counter.count(ig_intr_md.ingress_port);
					#endif
				...
				if(meta.p4sta_metadata.udp_length_cutted >= 1) {
			...
			if(meta.p4sta_metadata.added_empty_header == 1w0x1 || meta.p4sta_metadata.timestamp2 == 1w0x1){
				count_stamped_ingress();
			}

			if(meta.p4sta_metadata.timestamp2 == 1w0x1){
				...
				meta.p4sta_metadata.do_multicast = multi_counter_register_action.execute(0);

				if(meta.p4sta_metadata.do_multicast == 0x1){
					#ifdef P4STA_DEBUG_COUNTERS
					dbg_timestamp2_mc_counter.count(ig_intr_md.ingress_port);
					#endif
		...
		if(meta.p4sta_metadata.added_empty_header == 1w0x1){
			#ifdef PPPOE_ENCAP
...
control SwitchIngressDeparser(packet_out packet, inout headers_t hdr, in my_metadata_t meta, in ingress_intrinsic_metadata_for_deparser_t ig_dprsr_md) {
	...
	apply {
		if(meta.p4sta_metadata.updateChecksum == 1w0x1){
			#ifdef GTP_ENCAP
			//GTP-U case
			...
			hdr.tcp.checksum = tcp_checksum.update({
				...
				hdr.tcp.flags,
				hdr.tcp.srcPort,
				meta.p4sta_metadata.checksum_l4_tmp,
...
parser SwitchEgressParser(packet_in packet, out headers_t hdr, out my_metadata_t meta, out egress_intrinsic_metadata_t eg_intr_md) {
	...
	state start {
		...
		meta.l2_header_offset = 0;
		// session_identifier can be either PPPoE session ID, GTP-U TEID (downstream), is 0 if upstream
		meta.session_identifier32 = 32w0;
		meta.session_identifier16 = 16w0;
		// for upstream the subscriber source IP is used for session identification
		meta.session_identifier_ip = 32w0;
		meta.logical_index = 0;

		#ifdef DO_METER
		meta.meter_color = (bit<8>)MeterColor_t.GREEN;
		#endif
	...
	state parse_outer_ipv4 {
		packet.extract(hdr.outer_ipv4);
		meta.p4sta_metadata.outer_ipv4_paketlen_temp = hdr.outer_ipv4.paketlen;
	...
	state parse_outer_udpGtpu {
		packet.extract(hdr.outer_udp);
		packet.extract(hdr.gtpu);
		meta.session_identifier32 = hdr.gtpu.teid;
	...
	state parse_vlan {
		packet.extract(hdr.vlan);
		meta.l2_header_offset = 4; //TODO: what if e.g. vlan + PPPoE?
		transition select(hdr.vlan.etherType) {
			16w0x0800: parse_ipv4;
			16w0x8100: parse_qinq_vlan;
			#ifdef PPPOE_ENCAP
	...
	state parse_qinq_vlan {
		packet.extract(hdr.vlan_qinq);
		meta.l2_header_offset = 8; //TODO: what if e.g. vlan + PPPoE?
		transition select(hdr.vlan_qinq.etherType) {
			16w0x0800: parse_ipv4;
			#ifdef PPPOE_ENCAP
			16w0x8864: parse_pppoe;
	...
	state parse_pppoe {
        packet.extract(hdr.pppoe);
		meta.session_identifier16 = hdr.pppoe.sessionID;
	...
	state parse_ipv4 {
		packet.extract(hdr.ipv4);
		tcp_checksum.subtract({hdr.ipv4.paketlen});
		tcp_checksum.subtract({hdr.ipv4.srcAddr});
		tcp_checksum.subtract({hdr.ipv4.dstAddr});
		meta.p4sta_metadata.ipv4_paketlen_temp = hdr.ipv4.paketlen;
		meta.session_identifier_ip = hdr.ipv4.srcAddr; // use source IP for session identification for upstream packets (is still set for downstream but ignored)
		transition select(hdr.ipv4.protocol) {
			8w0x6: parse_tcp;
			8w0x11: parse_udp;
			8w0x1: parse_icmp; //TODO: parse when in vlan pppoe gtpu
...
control SwitchEgress(
	...
	#ifdef DO_METER
	DirectMeter(MeterType_t.BYTES) direct_shaper;
	#endif

	RegisterParam<bit<32>>(0) iterator_end; //sets end of local iterator for session mapping
	Register<bit<32>, bit<9>>(512, 0) iterator;
	RegisterAction<bit<32>, bit<9>, bit<32>>(iterator) iterator_incr_action = {
		void apply(inout bit<32> value, out bit<32> output){
				if(value >= iterator_end.read()) {
							value = 0;
							output = value;
					} else {
							output = value;
	...
	action increment_local_index() {
		meta.logical_index = iterator_incr_action.execute(eg_intr_md.egress_port);
	...
	#include "session_mod_actions.p4"
...
///////// tables /////////
	...
	table t_check_local_index_increment {
		key = {
			eg_intr_md.egress_port : exact;
		}
		actions = {
		...
		size = 8; //512 is size of bit<9> egress port, limit for ressource efficiency
	}
	...
	table t_session_mapping {
		key = {
			meta.logical_index : exact;
			eg_intr_md.egress_port : exact;
		}
		actions = {
			// to save stages instead of 1 table for each of the fields but still enables all combinations of fields
			// ethernet and ip always combines src and dst (=> "mod_field*S*"), also for saving ressources
			// mod_fields_ethernet;
			// mod_field_vlan;
			// mod_field_vlan_qinq;
			// mod_field_pppoe;
			...
			// mod_fields_ethernet_vlan;
			// mod_fields_ethernet_vlan_qinq;
			// mod_fields_ethernet_pppoe; // upstream => removed as pppoe required ip modification too
			mod_fields_ethernet_ipv4; // downstream
			//mod_fields_vlan_vlan_qinq;
			//mod_fields_vlan_pppoe;
			//mod_fields_vlan_ipv4;
			//mod_fields_vlan_qinq_pppoe;
			//mod_fields_vlan_qinq_ipv4;
			//mod_fields_pppoe_ipv4;

			// mod_fields_ethernet_vlan_vlan_qinq; // downstream => removed as IP change required in downstream
			// mod_fields_ethernet_vlan_pppoe; // upstream => can be removed as pppoe change requires ip change
			mod_fields_ethernet_vlan_ipv4; // downstream
			// mod_fields_ethernet_vlan_qinq_pppoe; // no single vlan_qinq header, needs normal vlan header before
			mod_fields_ethernet_vlan_qinq_ipv4; // downstream
			mod_fields_ethernet_pppoe_ipv4; // upstream
			// mod_fields_vlan_vlan_qinq_pppoe; // upstream => can be removed as pppoe change requires mac change
			// mod_fields_vlan_vlan_qinq_ipv4; // downstream => can be removed as no mac change
			// mod_fields_vlan_pppoe_ipv4; // upstream => can be removed as pppoe change requires mac change
			// mod_fields_vlan_qinq_pppoe_ipv4; // upstream => can be removed as pppoe change requires mac change

			// mod_fields_ethernet_vlan_vlan_qinq_pppoe; upstream => can be removed as pppoe change requires ip change
			mod_fields_ethernet_vlan_vlan_qinq_ipv4; // downstream
			mod_fields_ethernet_vlan_pppoe_ipv4; // upstream vlan single tagged
			// mod_fields_ethernet_vlan_qinq_pppoe_ipv4; // no single vlan_qinq header, needs normal vlan header before
			// mod_fields_vlan_vlan_qinq_pppoe_ipv4; // upstream => removed as MAC modification required

			mod_fields_ethernet_vlan_vlan_qinq_pppoe_ipv4; // upstream with qinq


			// mit "shaper" meter
			// mod_fields_ethernet_ipv4_shaper;
			// mod_fields_ethernet_vlan_ipv4_shaper; // downstream
			// mod_fields_ethernet_vlan_qinq_ipv4_shaper; // downstream
			// mod_fields_ethernet_pppoe_ipv4_shaper;
			// mod_fields_ethernet_vlan_vlan_qinq_ipv4_shaper; // downstream
			// mod_fields_ethernet_vlan_pppoe_ipv4_shaper;
			// mod_fields_ethernet_vlan_vlan_qinq_pppoe_ipv4_shaper; 

		}
		size = LOG_INDX_MAX_SIZE;
		#ifdef DO_METER
		meters = direct_shaper;
		#endif
	}
	...
	apply {
		...
		if(hdr.ipv4.isValid()){
			...
			if (hdr.ext_host_stats.isValid()){
				...
				#if GTP_ENCAP
				if(meta.session_identifier32 != 0){ // downstream => sess header is added by DUT
					hdr.ext_host_stats.sess_identifier = meta.session_identifier32;
				} else {							// upstream => sess header is removed by DUT
					hdr.ext_host_stats.sess_identifier = meta.session_identifier_ip;
				}
				#else
					// GTP and PPPOE are mutually exclusive!
					#if PPPOE_ENCAP
					if(meta.session_identifier16 != 0){ // downstream => sess header is added by DUT
						hdr.ext_host_stats.sess_identifier = (bit<32>)meta.session_identifier16;
					} else {							// upstream => sess header is removed by DUT
						hdr.ext_host_stats.sess_identifier = meta.session_identifier_ip;
					}

					#else
					// no session identifier when compiled without session flags
					// prevents collection of random src IPs to reduce results storage
					hdr.ext_host_stats.sess_identifier = 32w0x0;
					#endif
				#endif
				...
				if(hdr.vlan.isValid()){
						if(hdr.pppoe.isValid()){
							hdr.ext_host_stats.paket_len = meta.p4sta_metadata.ipv4_paketlen_temp + 26; // + 14 byte ethernet() and + 8 byte PPPoE() + 4 byte VLAN()
						} else {
							...
							#ifdef GTP_ENCAP
							hdr.ext_host_stats.paket_len = meta.p4sta_metadata.outer_ipv4_paketlen_temp + 18; // + 14 byte ethernet() + 4 byte VLAN()
							#else
							hdr.ext_host_stats.paket_len = meta.p4sta_metadata.ipv4_paketlen_temp + 18; // + 14 byte ethernet() + 4 byte VLAN()
							#endif
						}
				} else {
						if(hdr.pppoe.isValid()){
							hdr.ext_host_stats.paket_len = meta.p4sta_metadata.ipv4_paketlen_temp + 22; // + 14 byte ethernet() and + 8 byte PPPoE()
						} else {
							#ifdef GTP_ENCAP
							if(meta.p4sta_metadata.outer_ipv4_paketlen_temp > 0){
								hdr.ext_host_stats.paket_len = meta.p4sta_metadata.outer_ipv4_paketlen_temp + 14; // + 14 byte ethernet()
							} else {
								// Case: IP packet without encap (e.g. after egressing UPF upstream)
								hdr.ext_host_stats.paket_len = meta.p4sta_metadata.ipv4_paketlen_temp + 14; // + 14 byte ethernet()
							}
							#else
							hdr.ext_host_stats.paket_len = meta.p4sta_metadata.ipv4_paketlen_temp + 14; // + 14 byte ethernet()
							#endif
		...
		if(hdr.ipv4.isValid()) {
			...
			// add vlan offsets to header_offset
			meta.header_offset = meta.header_offset + meta.l2_header_offset;
			...
			if( (hdr.tcp_options_128bit_custom.isValid() && hdr.tcp_options_128bit_custom.myType == 16w0x0f10) || ( hdr.tcp_options_128bit_custom_without.isValid() && hdr.tcp_options_128bit_custom_without.myType == 16w0x0f10)){
				...
				t_check_local_index_increment.apply();
				t_session_mapping.apply();
			}
			#else
		...
		// t_session_mapping.apply(); => rewrites LCP packets too?
		#ifdef DO_METER
		if (meta.meter_color == (bit<8>)MeterColor_t.RED) {
            // drop packet
...
control SwitchEgressDeparser(packet_out packet, inout headers_t hdr, in my_metadata_t meta, in egress_intrinsic_metadata_for_deparser_t eg_dprsr_md) {
	...
	apply {
		if(meta.p4sta_metadata.updateEgressIPChecksum == 1w0x1){
		...
		if(meta.p4sta_metadata.updateChecksum == 1w0x1){
			hdr.tcp.checksum = tcp_checksum.update({
				hdr.ipv4.srcAddr, // can be modified for session flows
				hdr.ipv4.dstAddr, // can be modified for session flows
				hdr.ipv4.paketlen,
				hdr.tcp.dataOffset,
				hdr.tcp.res,
				hdr.tcp.flags,
