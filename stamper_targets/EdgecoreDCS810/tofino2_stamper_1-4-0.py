...
try:
    ...
    from tofino1_65p_stamper_v1_4_0 import TargetImpl as tofino1Impl
...
class TargetImpl(tofino1Impl):
    ...
    def start_stamper_software(self, cfg):
        ...
        for i in range(12):
            ...
            self.logger.info("Probing gRPC connection to Tofino 2... Established: " + str(established))
            interface.teardown()
            if established:
                reachable = True
                break
        ...
        if reachable:
            self.logger.info("Tofino 2 gRPC server reachable.")
        else:
            self.logger.error("Tofino 2 gRPC server not reachable after 2 minutes of probing.")
